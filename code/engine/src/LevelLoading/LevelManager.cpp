...
namespace gl3::engine::levelLoading
{
    ...
    Level* LevelManager::loadLevel(const int ID, const std::string& filename)
    {
        ...
        const auto savePath = std::filesystem::path(resolveSavePath("levels")) / filename;
        const auto assetPath = std::filesystem::path(resolveAssetPath("levels")) / filename;
    ...
    void LevelManager::saveCurrentLevel()
    {
        ...
        const auto path = std::filesystem::path(resolveSavePath("levels")) / filenameIt->second;
