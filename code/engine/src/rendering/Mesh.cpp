/**
* @file Mesh.cpp
 * @brief Implements the Mesh class for storing and drawing vertex and index data.
 */
...
{
    template <typename T>
    GLuint createBuffer(const GLuint bufferType, const std::vector<T>& bufferData)
    {
        ...
        EBO(createBuffer(GL_ELEMENT_ARRAY_BUFFER, indices))
    {
        ...
        // Set up position attribute: location 0, 3 floats, stride of 5 floats.
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), nullptr);
        glEnableVertexAttribArray(0);

        // Set up texture coordinate attribute: location 1, 2 floats, offset by 3 floats.
