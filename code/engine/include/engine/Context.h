...
namespace gl3::engine::context {
    ...
    public:
        ...
        events::Event<Context> onFrameBufferSizeChange;
        events::Event<Context, const double> onScrolling;

        explicit Context(int width = 800, int height = 600, const std::string &title = "Game", glm::vec3 camPos = glm::vec3(0.0f, 0.0f, 0.0f), float camZoom = 1.0f);
        virtual ~Context();
