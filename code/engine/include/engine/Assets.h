...
namespace gl3::engine
{
    ...
    /**
     * This utility joins the `SAVE_ROOT` macro with a relative level save path
     * and returns the absolute, normalized path as a string.
     *
     * @param subPath The relative path inside the save directory
     * @return The absolute path corresponding to the save location
     */
    inline std::string resolveSavePath(const std::string& subPath) {
        return getExecutablePath().parent_path().string() + "/" + std::string(SAVE_ROOT) + "/" + subPath;
    }
}
